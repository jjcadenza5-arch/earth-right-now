...
const now=Date.now(),fresh=s=>{const d=Date.parse(s.lastSuccessfulCheck||s.checkedAt||"");return Number.isFinite(d)&&d<=now+5*60*1000&&(now-d)<=21*86400000};
...
if(fail.length){console.error(JSON.stringify({ok:false,fail,metrics:{eligible:eligible.length,inside:inside.length,countries:countries.size,providers:providers.size,truth:[...truth]}},null,2));process.exit(1)}
