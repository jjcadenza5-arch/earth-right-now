...
const [key,status,...noteParts]=process.argv.slice(2),note=noteParts.join(" ").trim();
if(!key||!status||!note){console.error("Usage: npm run release:record -- <key> <pass|fail> <evidence note>");process.exit(2)}
...
const next=evidencePatch(ledger,key,{ok,note});
await writeFile(url,JSON.stringify(next,null,2)+"\n","utf8");
console.log(`Recorded ${key}: ${ok?"PASS":"FAIL"}`);
