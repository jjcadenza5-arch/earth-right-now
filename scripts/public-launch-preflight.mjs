...
for(const [name,html,url] of [["index",index,"https://earthrightnow.app/"],["about",about,"https://earthrightnow.app/about.html"],["privacy",privacy,"https://earthrightnow.app/privacy.html"]]){assert(html.includes(`rel="canonical" href="${url}"`),`${name} canonical mismatch`);assert(!html.includes("github.io"),`${name} must not advertise GitHub hostname`)}
...
for(const p of ["manifest.webmanifest","service-worker.js","offline.html","sitemap.xml","robots.txt","CNAME","about.html","privacy.html"])assert(build.includes(`../${p}`),`release artifact missing ${p}`);
...
console.log(JSON.stringify({ok:true,domain:"earthrightnow.app",trustPages:["about.html","privacy.html"],pwa:true,offlineFallback:true},null,2));
