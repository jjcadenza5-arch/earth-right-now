...
const candidates=(await Promise.all(files.map(read))).flat();
const active=new Map(sources.map(x=>[x.id,x]));
const canonical=id=>String(id||"").replace(/-recovered$/,"");
const migrated=[],pending=[];
for(const candidate of candidates){const id=canonical(candidate.id),source=active.get(id);if(source)migrated.push({candidateId:candidate.id,sourceId:id,title:source.title,health:source.health,checkedAt:source.checkedAt||null});else pending.push({candidateId:candidate.id,canonicalId:id,title:candidate.title,health:candidate.health,permission:candidate.permission,playback:candidate.playback,checkedAt:candidate.checkedAt||null});}
console.log(JSON.stringify({activeSources:sources.length,recoveryCandidates:candidates.length,migrated:migrated.length,pending:pending.length,pendingCandidates:pending},null,2));
