...
assert.equal(r.summary.permissionReview,1);assert.equal(r.summary.humanMediaReview,1);assert.equal(r.summary.manualSourceReview,1);assert.equal(r.summary.accessLimited,1);assert.equal(r.summary.editorialRecheck,1);assert.equal(r.summary.retryLater,1);
assert.ok(r.immediate.some(x=>x.id==="permission"));assert.ok(r.immediate.some(x=>x.id==="degraded"));assert.equal(r.safety.catalogMutationAllowed,false);assert.equal(r.safety.automaticHealthChangeAllowed,false);assert.equal(r.safety.availabilityProvesLive,false);
