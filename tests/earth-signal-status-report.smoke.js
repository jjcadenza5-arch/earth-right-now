...
const r=earthSignalStatusReport();assert.equal(r.mode,"READ_ONLY");assert.equal(r.ready,false);assert.equal(r.blockers.length,6);assert.equal(r.truth,"Architecture readiness and an API contract are not production capability.");assert.equal(r.backendContract.version,"2026-09-25.v1");assert.equal(r.backendContract.ttlMinutes,45);
