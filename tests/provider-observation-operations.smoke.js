...
assert.equal(r.healthAutomation.changes,0);assert.equal(r.healthAutomation.inconclusive,1);assert.equal(r.healthAutomation.confirmedHealthy,0);assert.deepEqual(r.healthAutomation.outcomeDetails.inconclusive,[{id:"cam-a",observedAt:now,reason:"Page reachable but current media not confirmed"}]);
