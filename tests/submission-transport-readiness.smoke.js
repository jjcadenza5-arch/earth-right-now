...
let r=submissionTransportReadiness({enabled:false,endpoint:"",privacyUrl:"https://earthrightnow.app/privacy.html",retentionDays:null});
assert.equal(r.status,"DISABLED");assert.equal(r.active,false);assert.ok(r.missing.includes("HTTPS_REVIEW_ENDPOINT"));assert.ok(r.missing.includes("RETENTION_POLICY"));assert.equal(r.privacyReady,true);
