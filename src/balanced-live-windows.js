...
export function balancedLiveWindows(sources,{limit=8,maxPerCountry=2,maxPerPlace=1,now=new Date()}={}){
 const pool=(sources||[]).filter(s=>discoverableSource(s)&&currentSource(s)&&playbackCapability(s).action!=="UNAVAILABLE").sort((a,b)=>beautifulNowScore(b,now)-beautifulNowScore(a,now));
