import { discoverableSource } from "./discovery-eligibility.js";import { nearNowEvidence } from "./now-evidence.js";import { playbackCapability } from "./playback-capability.js";import { beautifulNowScore } from "./solar-moment.js";
export function balancedLiveWindows(sources,{limit=8,maxPerCountry=2,maxPerPlace=1,now=new Date()}={}){
 const pool=(sources||[]).filter(s=>discoverableSource(s)&&nearNowEvidence(s,{now})&&playbackCapability(s,{now}).action!=="UNAVAILABLE").sort((a,b)=>beautifulNowScore(b,now)-beautifulNowScore(a,now));
