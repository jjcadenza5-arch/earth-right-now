...
const currentWords=new Set(["now","live","current","today","tonight","ตอนนี้","สด"]);
...
export function searchEarth(sources,q,{now=new Date()}={}){
 const terms=norm(q).split(/\s+/).filter(Boolean),intent=interpretEarthIntent(q),wantsCurrent=intent.wantsCurrent||terms.some(t=>currentWords.has(t)),intentWords=new Set(intent.intents.flatMap(i=>({wildlife:["wildlife","animal","animals","elephant","bird","birds","zoo","nature"],water:["beach","beaches","sea","ocean","coast","coastal","water","surf","harbour","harbor"],mountains:["mountain","mountains","volcano","volcanoes","snow","summit","alpine"],human:["city","cities","street","streets","people","urban","square","harbour","harbor"],beautiful:["beautiful","scenic","view","views","relax","relaxing","calm","peaceful"],happening:["happening","active","event","busy","crowd","crowds"],night:["night","nighttime","lights","after","dark"],daylight:["daylight","sunny","daytime"],golden:["sunrise","sunset","golden","hour","dawn","dusk"],reference:["photo","photos","image","images","reference","picture","pictures"]}[i]||[]))),semanticTerms=terms.filter(t=>!currentWords.has(t)&&!stopWords.has(t)&&!intentWords.has(t)&&!(intent.wantsCurrent&&(t==="ตอนนี้"||t==="สด"))),pool=sources.filter(s=>wantsCurrent?nearNowEvidence(s,{now}):discoverableSource(s));
