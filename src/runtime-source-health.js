...
export function rememberRuntimeFailure(source,failure,now=Date.now()){now=time(now);const s=store();if(!s||!source?.id)return;const map=loadRuntimeFailures(now);map.set(source.id,{id:source.id,at:now,kind:failure?.kind||"PLAYBACK_FAILED"});try{s.setItem(KEY,JSON.stringify([...map.values()]))}catch{}}
