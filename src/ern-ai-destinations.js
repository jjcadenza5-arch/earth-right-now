...
export function rankDestinationsForIntent(sources,query=""){
 const ranked=rankForIntent(sources,query),sourcePosition=new Map(ranked.map((s,i)=>[s.id,i]));
 return groupByPlace(ranked).sort((a,b)=>{
  ...
  return ai-bi||destinationRank(b)-destinationRank(a);
 });
}
