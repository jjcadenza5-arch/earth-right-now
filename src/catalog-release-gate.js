...
export function catalogReleaseGate(rows,{minimumCurrentHealthy=1,minimumInsideERN=1,allowUnknown=true,now=Date.now(),...recencyOptions}={}){
 ...
 const currentHealthy=valid.filter(s=>currentSource(s,currentOptions)).length,currentInsideERN=valid.filter(s=>currentSource(s,currentOptions)&&playbackCapability(s).action==="PLAY").length,unknown=valid.filter(s=>s.health==="UNKNOWN").length;
