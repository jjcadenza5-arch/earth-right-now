...
export function discoveryStatus(result={}){
 ...
 if(result.empty)return result.currentIntent?"No verified-current destination matches yet. Try another place or search the destination without “live/now” to see other available or reference views.":"No matching destination yet. Try a place, country, landscape, wildlife, coast, mountain or city.";
 const liveLike=result.liveLikeCount??Math.max(0,result.windowCount-(result.referenceCount||0)),current=result.currentCount??liveLike,available=Math.max(0,result.availableNonCurrentCount??(liveLike-current));const parts=[result.count+" destination"+(result.count===1?"":"s")];if(result.nearNowCount)parts.push(result.nearNowCount+" near-now view"+(result.nearNowCount===1?"":"s"));else if(current)parts.push(current+" recently checked source"+(current===1?"":"s"));if(available)parts.push(available+" available view"+(available===1?"":"s")+" · current check not confirmed");if(result.referenceCount)parts.push(result.referenceCount+" reference "+(result.referenceCount===1?"image":"images"));return parts.join(" · ");
}
