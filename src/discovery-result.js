...
export function discoveryStatus(result={}){
 ...
 const liveLike=result.liveLikeCount??Math.max(0,result.windowCount-(result.referenceCount||0)),current=result.currentCount??liveLike,available=Math.max(0,result.availableNonCurrentCount??(liveLike-current));const parts=[result.count+" destination"+(result.count===1?"":"s")];if(current)parts.push(current+" current/live view"+(current===1?"":"s"));if(available)parts.push(available+" available view"+(available===1?"":"s")+" · current check not confirmed");if(result.referenceCount)parts.push(result.referenceCount+" reference "+(result.referenceCount===1?"image":"images"));return parts.join(" · ");
}
