...
export function discoveryResult(sources,query,{limit=12,mode="search",taste=null,now=new Date()}={}){
 ...
 const windowCount=items.reduce((n,p)=>n+(p.sources?.length||0),0),referenceCount=items.reduce((n,p)=>n+(p.sources||[]).filter(s=>s.truth==="PREVIEW"||s.playback==="PREVIEW").length,0),liveLikeCount=Math.max(0,windowCount-referenceCount);
 return{query:q,items,count:items.length,windowCount,liveLikeCount,referenceCount,empty:items.length===0,currentIntent:intent.wantsCurrent};
}
export function discoveryStatus(result={}){
 ...
 const liveLike=result.liveLikeCount??Math.max(0,result.windowCount-(result.referenceCount||0));const parts=[result.count+" destination"+(result.count===1?"":"s")];if(liveLike)parts.push(liveLike+" current/live view"+(liveLike===1?"":"s"));if(result.referenceCount)parts.push(result.referenceCount+" reference "+(result.referenceCount===1?"image":"images"));return parts.join(" · ");
}
