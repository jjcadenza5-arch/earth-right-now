...
export function windowTileView(source,index,{onOpen,now=new Date()}={}){
  ...
  b.append(visual,element("span",{className:"window-moment",text:watchEarthMomentLabel(source,now)}),element("strong",{text:x.title}),element("small",{text:x.meta}),element("small",{className:"window-truth-line",text:`${x.eyebrow} · ${freshnessCopy(source,{now})}`}));
