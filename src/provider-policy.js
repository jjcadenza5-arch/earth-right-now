const providers=[
 {match:/earthcam/i,kind:"external-live-provider",requiresProviderCheck:true,defaultPermission:"LINK_ONLY"},
...
export function providerPolicy(source){const url=[source.provider,source.sourceUrl,source.embedUrl,source.officialUrl].filter(Boolean).join(" ");return providers.find(p=>p.match.test(url))||{kind:"unknown",requiresProviderCheck:true,defaultPermission:"UNKNOWN"}}
