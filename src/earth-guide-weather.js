...
export function earthGuideWeatherBoundary(query,result={},options={}){
 ...
 if(verifiedWeather>0)return{canClaimCurrent:true,text:language==="en"?`ERN has ${verifiedWeather} current ${condition} observation${verifiedWeather===1?"":"s"} backed by explicit weather evidence.`:guideFormat("weatherNear",{count:verifiedWeather,plural:verifiedWeather===1?"":"s",condition},language)};
