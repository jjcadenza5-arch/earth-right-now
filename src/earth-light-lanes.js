...
export function earthLightLanes(sources,{now=new Date(),limitPerLane=4}={}){
 ...
 for(const s of sources||[]){if(!currentSource(s))continue;const m=solarMoment(s,now);if(m.phase==="NIGHT"&&!/(Cities|Harbour|Skyline|Urban|Streets|Landmarks)/i.test((s.categories||[]).join(" ")))continue;if(groups.has(m.phase))groups.get(m.phase).push({...s,solar:m})}
