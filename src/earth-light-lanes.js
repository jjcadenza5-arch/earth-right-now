...
const ORDER=["SUNRISE","SUNSET","MORNING_GOLDEN","EVENING_GOLDEN","NIGHT","DAY"];
...
export function earthLightLanes(sources,{now=new Date(),limitPerLane=4}={}){
 ...
 for(const s of sources||[]){if(!currentSource(s))continue;const m=solarMoment(s,now);if(groups.has(m.phase))groups.get(m.phase).push({...s,solar:m})}
