...
export function earthGuideReply(result,{tasteSignals=0}={}){
 ...
 const current=intent.wantsCurrent?"current ":"",fallback=!intent.wantsCurrent&&result.referenceCount?` ${result.referenceCount} reference ${result.referenceCount===1?"image is":"images are"} clearly labeled, not live.`:"",personal=tasteSignals?" I also used your local My Earth taste as a gentle preference, after your request.":"";
 return{tone:"FOUND",text:`I found ${result.count} ${current}destination${result.count===1?"":"s"}${theme?` for ${theme}`:""}. Start with ${where}.${fallback}${personal}`};
}
