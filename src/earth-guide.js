...
export function earthGuideReply(result,{tasteSignals=0}={}){
 ...
 const current=intent.wantsCurrent?"current ":"",evidence=!intent.wantsCurrent&&result.nearNowCount?` ${result.nearNowCount} near-now ${result.nearNowCount===1?"view is":"views are"} supported by current evidence.`:"",unchecked=!intent.wantsCurrent&&result.availableNonCurrentCount?` ${result.availableNonCurrentCount} additional ${result.availableNonCurrentCount===1?"view is":"views are"} available but its current check is not confirmed.`:"",fallback=!intent.wantsCurrent&&result.referenceCount?` ${result.referenceCount} reference ${result.referenceCount===1?"image is":"images are"} clearly labeled as not current.`:"",personal=tasteSignals?" I also used your local My Earth taste as a gentle preference, after your request.":"";
