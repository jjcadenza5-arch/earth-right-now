...
export function earthGuideReply(result,{tasteSignals=0}={}){
 ...
 if(result?.empty)return{tone:"EMPTY",text:intent.wantsCurrent?"I couldn't find a verified-current match for that yet. Try the same place without “live” to see clearly labeled available or reference views.":"I don't have a good match for that yet. Try a place, landscape, city, coast, mountain or wildlife view."};
 ...
 const current=intent.wantsCurrent?"current ":"",evidence=!intent.wantsCurrent&&result.currentCount?` ${result.currentCount} current/live ${result.currentCount===1?"view is":"views are"} verified now.`:"",unchecked=!intent.wantsCurrent&&result.availableNonCurrentCount?` ${result.availableNonCurrentCount} additional ${result.availableNonCurrentCount===1?"view is":"views are"} available but its current check is not confirmed.`:"",fallback=!intent.wantsCurrent&&result.referenceCount?` ${result.referenceCount} reference ${result.referenceCount===1?"image is":"images are"} clearly labeled, not live.`:"",personal=tasteSignals?" I also used your local My Earth taste as a gentle preference, after your request.":"";
 const currentEvidence=intent.wantsCurrent&&result.currentCount?` ${result.currentCount} current/live ${result.currentCount===1?"view is":"views are"} verified now.`:"";return{tone:"FOUND",text:`I found ${result.count} ${current}destination${result.count===1?"":"s"}${theme?` for ${theme}`:""}. Start with ${where}.${currentEvidence}${evidence}${unchecked}${fallback}${personal}`};
}
