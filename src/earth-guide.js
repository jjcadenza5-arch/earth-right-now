...
export function earthGuideReply(result,{tasteSignals=0}={}){
 ...
 return{tone:"FOUND",text:`I found ${result.count} ${current}destination${result.count===1?"":"s"}${theme?` for ${theme}`:""}. Start with ${where}.${evidence}${unchecked}${fallback}${personal}`};
}
