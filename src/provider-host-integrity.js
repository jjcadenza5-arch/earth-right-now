function safeUrl(value){
 try{const u=new URL(String(value||""));return u.protocol==="https:"&&!u.username&&!u.password?u:null}catch{return null}
}
function rootHost(host){const h=String(host||"").toLowerCase().replace(/^www\./,"");const parts=h.split(".");return parts.length>2?parts.slice(-2).join("."):h}
export function providerHostIntegrity(source){
 const sourceUrl=safeUrl(source?.sourceUrl),embedUrl=safeUrl(source?.embedUrl),officialUrl=safeUrl(source?.officialUrl);
 const issues=[];
