import { heroPosterStyle,posterSceneMeta } from "./source-poster.js";
export function applyHeroPoster(media,source,{imageFactory=()=>document.createElement("img")}={}){
 ...
 if(!visual.backgroundImage){const scene=posterSceneMeta(source);media.style?.setProperty?.("--poster-x",scene.positionX+"%");media.style?.setProperty?.("--poster-y",scene.positionY+"%");media.setAttribute?.("data-poster-variant",String(scene.variant));return{kind:"generated",className:visual.className};}
