...
export function reviewSubmission(record,{decision,reviewedAt=new Date().toISOString(),note="",checks={}}={}){
 ...
 const required=SUBMISSION_REVIEW_CHECK_IDS,missing=required.filter(k=>checks?.[k]!==true),cleanNote=String(note||"").trim().slice(0,500),reviewTime=Date.parse(reviewedAt);\n if(!Number.isFinite(reviewTime))return{ok:false,error:"Review timestamp must be valid"};
 if(decision==="APPROVED"&&missing.length)return{ok:false,error:"Required source review checks are incomplete",missing};
 return{ok:true,record:{...record,status:decision,reviewedAt:new Date(reviewTime).toISOString(),note:cleanNote||null,reviewChecks:Object.fromEntries(required.map(k=>[k,checks?.[k]===true]))}};
}
export function submissionCanPublish(record){return Boolean(record?.status===STATUS.APPROVED&&record?.rightsConfirmed===true&&record?.sourceUrl&&SUBMISSION_REVIEW_CHECK_IDS.every(k=>record?.reviewChecks?.[k]===true));}
