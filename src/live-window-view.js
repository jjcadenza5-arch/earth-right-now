...
export function liveWindowView(source,{onOpen,index=0}={}){
  ...
  const moment=element("span",{className:"live-window-moment",text:watchEarthMomentLabel(source)}),evidence=element("span",{className:"window-evidence",text:windowEvidenceTier(source).label}),top=element("span",{className:"live-window-state",text:currentWindowEyebrow(source)}),title=element("strong",{text:source.title}),meta=element("small",{text:[source.region,source.country].filter(Boolean).join(" · ")}),fresh=element("small",{className:"source-freshness",text:freshnessCopy(source)}),badges=element("span",{className:"live-window-badges",text:sourceBadges(source).join(" · ")});
