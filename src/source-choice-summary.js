...
export function sourceChoiceCounts(place){
 const sources=(place?.sources||[]).filter(s=>discoverableSource(s)&&playbackCapability(s).action!=="UNAVAILABLE"&&s.playback!=="PREVIEW"),inside=sources.filter(s=>playbackCapability(s).action==="PLAY").length,external=sources.filter(s=>playbackCapability(s).action==="EXTERNAL").length;
