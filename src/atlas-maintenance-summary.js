...
export function atlasMaintenanceSummary(sources=[], {now=new Date(),limit=10}={}){
 ...
 const intentionalDynamic=sources.filter(s=>s.mapBehavior==="DYNAMIC_UNPINNED");
 const unmapped=sources.filter(s=>s.mapBehavior!=="DYNAMIC_UNPINNED"&&(!Number.isFinite(s.lat)||!Number.isFinite(s.lon))).map(s=>({id:s.id,title:s.title,current:currentSource(s,{now}),score:score(s),health:s.health,provider:s.provider||null,action:"VERIFY_PLACE_COORDINATES_FROM_AUTHORITATIVE_SOURCE"})).sort(sort);
