...
export function atlasMaintenanceSummary(sources=[], {now=new Date(),limit=10}={}){
 ...
 const unmapped=sources.filter(s=>!Number.isFinite(s.lat)||!Number.isFinite(s.lon)).map(s=>({id:s.id,title:s.title,current:currentSource(s,{now}),score:score(s),health:s.health,provider:s.provider||null,action:"VERIFY_PLACE_COORDINATES_FROM_AUTHORITATIVE_SOURCE"})).sort(sort);
