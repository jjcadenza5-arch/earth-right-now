...
import { applyPoster } from "./source-poster.js";import { sourceChoiceSummary } from "./source-choice-summary.js";
...
export function destinationCardView(place,{onOpen,context=""}={}){
  ...
  const copy=element("div"),meta=element("span",{className:"meta",text:place.country||""}),title=element("h3",{text:place.title}),region=element("p",{text:place.region||""}),status=x.current?x.current+" current"+(x.current===1?" window":" windows"):x.windows?x.windows+" available "+(x.windows===1?"source":"sources")+" · current check not confirmed":"No available windows",count=element("span",{className:"count",text:status+(x.hasChoice?" · choose your view":"")}),actions=element("div",{className:"card-actions"}),choice=sourceChoiceSummary(place),openLabel=x.hasChoice?"Choose a window":x.windows===1?"Open window":"Unavailable",open=element("button",{text:openLabel,attrs:{"aria-label":x.windows?openLabel+" for "+place.title+" — "+choice:"Unavailable for "+place.title}});
