...
export function destinationCardView(place,{onOpen,context="",now=new Date()}={}){
  ...
  const answer=placeAnswer(place,{now}),copy=element("div"),meta=element("span",{className:"meta",text:place.country||""}),title=element("h3",{text:place.title}),region=element("p",{text:place.region||""}),status=x.current?x.current+" current"+(x.current===1?" window":" windows"):x.windows?x.windows+" available "+(x.windows===1?"view":"views")+" · current check not confirmed":"No available views",count=element("span",{className:"count",text:status+(x.hasChoice?" · choose your view":"")}),actions=element("div",{className:"card-actions"}),choice=sourceChoiceSummary(place,{now}),action=destinationActionSemantics(place,x,choice),open=element("button",{text:action.label,attrs:{"aria-label":action.aria}});
