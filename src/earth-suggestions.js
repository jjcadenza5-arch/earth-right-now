...
export function earthSuggestions({currentAvailable=true,limit=5,includeRecent=true}={}){
 const recent=includeRecent?recentEarthSearches().map(query=>({label:`Again: ${query}`,query,intents:[],current:false,recent:true})):[];const defaults=SUGGESTIONS.filter(x=>currentAvailable||!x.current).map(x=>({...x,intents:[...x.intents]}));const recentCap=Math.min(recent.length,Math.max(0,Math.min(2,limit-3))),freshCap=Math.max(0,limit-recentCap);return [...defaults.slice(0,freshCap),...recent.slice(0,recentCap)].slice(0,Math.max(0,limit));
}
