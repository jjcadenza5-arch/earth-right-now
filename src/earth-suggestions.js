...
export function earthSuggestions({currentAvailable=true,limit=5,includeRecent=true}={}){
 const recent=includeRecent?recentEarthSearches().filter(query=>!SUGGESTIONS.some(x=>x.query.toLowerCase()===query.toLowerCase())).map(query=>({label:`Again: ${query}`,query,intents:[],current:false,recent:true})):[];const defaults=SUGGESTIONS.filter(x=>currentAvailable||!x.current).map(x=>({...x,intents:[...x.intents]}));const recentCap=Math.min(recent.length,Math.max(0,Math.min(2,limit-3))),freshCap=Math.max(0,limit-recentCap),fresh=defaults.slice(0,freshCap);if(currentAvailable&&limit>=5&&!fresh.some(x=>x.intents.includes("golden"))&&limit<7){const golden=defaults.find(x=>x.intents.includes("golden"));if(golden&&fresh.length)fresh[fresh.length-1]=golden}return [...fresh,...recent.slice(0,recentCap)].slice(0,Math.max(0,limit));
}
